const id = "DYNPROJ_EVENT_LAUNCH_PROJECTILE_SLOT";
const name = "Launch Projectile From Slot (Extended)";
...
const fields = [
  ...
  {
    label:
      "Same as the stock Launch Projectile From Slot event, but the slot is a plain number so it can reach the extra slots added by the Max projectile slots setting. The Dynamic projectile tab carries the few values that are per launch rather than per slot.",
  },
  {
    key: "slot",
  ...
  },
  {
    ...
    label: "Head Actor",
    description:
      "The actor the tail follows. The projectile itself is not drawn - this actor is the head - and it stops moving, colliding with tiles and expiring off screen, since all of that belongs to the actor now.",
    type: "actor",
    defaultValue: "$self$",
    conditions: [
      paramsTab,
  ...
  },
  {
    ...
    description:
      "The actor it is pinned to. Each anchored projectile carries its own, so several can hang off different actors at once.",
    type: "actor",
    defaultValue: "$self$",
    conditions: [paramsTab, { key: "paramBehaviour", in: ["anchor"] }],
  },
...
const compile = (input, helpers) => {
  ...
  if (input.directionType === "direction") {
    ...
    if (input.actorId === input.otherActorId) {
      launchProjectileInSourceActorDirection(slot, input.x, input.y);
    } else {
      launchProjectileInActorDirection(
        slot,
        input.x,
