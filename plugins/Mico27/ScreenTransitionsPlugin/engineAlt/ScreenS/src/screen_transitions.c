...
// === engineAlt variant: ScreenScroll / ContinuousScene ===
// These plugins offset the visible background inside VRAM by bkg_offset_x/y
// (hardware scroll = draw_scroll + TILE_TO_PX(bkg_offset)). Only the background
// VRAM WRITE coordinate is shifted by that offset; tilemap reads keep logical
// scene-tile coordinates. Otherwise identical to the base screen_transitions.c.
...
#ifdef CGB
        if (_is_CGB && tr_attr_ptr) tr_set_attr(vx, vy, ReadBankedUBYTE(tr_attr_ptr + off, tr_attr_bank));
#endif
        tr_set_tile(vx, vy, tile);
    }
}
