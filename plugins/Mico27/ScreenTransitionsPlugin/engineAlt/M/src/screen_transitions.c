...
// === engineAlt variant: MetaTile ===
// MetaTile keeps stock VRAM addressing, but a metatile scene stores a
// metatile-INDEX map in image_ptr. The REFRESH path therefore resolves each
// visible tile through the live SRAM metatile map (sram_map_data) and the
// metatile definitions (metatile_ptr), mirroring load_metatile_row() in the
// MetaTile scroll.c. FILL / overlay / copy+mask from NORMAL scenes are
// unchanged. (Copy/mask SOURCES should be normal scenes, not metatile ones.)
...
static void tr_put(UBYTE lx, UBYTE ly) {
    UBYTE sx = tr_x0 + lx;
    UBYTE sy = tr_y0 + ly;
    // The window write coordinate is region-relative (the overlay is not
    // scrolled); the background one tracks the live scroll (read here per tile
    // instead of caching it in globals). Tilemap READS are always scene-tile
    // coordinates, so the scroll is folded into sx/sy for BOTH layers.
    UBYTE vx = sx & 31u;
    UBYTE vy = sy & 31u;
    sx += (UBYTE)(scroll_x >> 3); sy += (UBYTE)(scroll_y >> 3);
    if (tr_layer == L_BKG) { vx = sx & 31u; vy = sy & 31u; }
