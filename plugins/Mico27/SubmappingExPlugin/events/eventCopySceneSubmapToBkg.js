...
export const compile = (input, helpers) => {
  ...
  const tmp6 = _declareLocal("tmp_ptr", 1, true);
  const tmp7 = _declareLocal("tmp_bank", 1, true);
  ...
  if (input.use_far_ptr){
	variableSetToScriptValue(tmp7, input.scene_bank);
	variableSetToScriptValue(tmp6, input.scene_ptr);	
  } 
  if (input.use_far_ptr){
	  _stackPush(tmp6);
	  _stackPush(tmp7);
