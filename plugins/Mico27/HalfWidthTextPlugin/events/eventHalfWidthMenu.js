const id = "EVENT_HWT_MENU";
const name = "Half-Width Text: Menu";
  ...
  [
    {
      ...
      label:
        ...
        "text renderer, so they come out in half-width characters like the rest of its " +
        "text. The stock Menu event draws its options at full width. Everything else -- " +
        "window size, cursor, navigation, the cancel flags -- behaves exactly as the " +
        "stock menu does, because a line of half-width text is one tilemap row tall " +
        "just like stock text.",
    },
    {
      key: "variable",
    ...
    },
    {
      ...
      description: "Each option takes one row, as stock text does. Half-width characters mean roughly twice as many fit across one.",
      type: "number",
      min: 2,
      max: 8,
      defaultValue: 2,
...
const compile = (input, helpers) => {
  ...
  _addComment("Half-Width Text: Menu");
