    ...
    if (ui_alt_text_ptr == 0) {
        ...
        if ((text_options & TEXT_OPT_PRESERVE_POS) == 0) {
            if (current_text_layer == TEXT_LAYER_BKG){
                ui_alt_dest_base = text_render_base_addr + ((((UWORD)bkg_offset_y << 5) + 32) & 1023) + (((UWORD)bkg_offset_x + 1) & 31);
            } else {
                ui_alt_dest_base = text_render_base_addr + 32 + 1;                  // gotoxy(1,1)
            }
    ...
    while (TRUE) {
        switch (*ui_alt_text_ptr) {
            ...
            case 0x03:
                // gotoxy
                if (current_text_layer == TEXT_LAYER_BKG){
                    ui_alt_dest_ptr = ui_alt_dest_base = text_render_base_addr + ((bkg_offset_x + (UWORD)(*++ui_alt_text_ptr - 1u)) & 31) + (((bkg_offset_y + (UWORD)(*++ui_alt_text_ptr - 1u)) << 5) & 1023);
                } else {
