...
const id = "EVENT_CAMERA_EX_LOCK_ACTOR_BY_INDEX";
const name = "Camera Lock On Actor By Index";
const groups = ["EVENT_GROUP_CAMERA"];
const subGroups = { EVENT_GROUP_CAMERA: "EVENT_GROUP_ACTOR" };

const autoLabel = (fetchArg) =>
  `Camera lock on actor index ${fetchArg("actorIndex")}`;
...
const fields = [
  {
    ...
    label: "Actor Index",
    description:
      "Scene index of the actor the camera will follow. The player is index 0.",
    type: "value",
...
const compile = (input, helpers) => {
  ...
  _addComment("Camera Lock On Actor By Index");
  compileTargets([{ actorIndex: input.actorIndex }], MEDIAN_BOUNDS, helpers);
...
module.exports = {
  ...
  name,
  description:
    "Locks the camera onto the actor at a given scene index, so the follow target can be chosen from a variable at runtime.",
  autoLabel,
  groups,
  subGroups,
  fields,
