...
const id = "EVENT_CAMERA_EX_LOCK_ACTOR";
const name = "Camera Lock On Actor";
const groups = ["EVENT_GROUP_CAMERA"];
const subGroups = { EVENT_GROUP_CAMERA: "EVENT_GROUP_ACTOR" };

const autoLabel = (fetchArg) => `Camera lock on ${fetchArg("actorId")}`;
...
const fields = [
  {
    ...
    label: "Actor",
    description:
      "Actor the camera will follow. Choosing the player reproduces the stock camera lock.",
    type: "actor",
    defaultValue: "$self$",
  },
...
const compile = (input, helpers) => {
  ...
  _addComment("Camera Lock On Actor");
  compileTargets([{ actorId: input.actorId }], MEDIAN_BOUNDS, helpers);
...
module.exports = {
  ...
  name,
  description:
    "Locks the camera onto any actor in the scene instead of the player, optionally easing towards it instead of snapping.",
  autoLabel,
  groups,
  subGroups,
  fields,
