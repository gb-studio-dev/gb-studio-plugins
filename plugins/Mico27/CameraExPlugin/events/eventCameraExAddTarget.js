...
const id = "EVENT_CAMERA_EX_ADD_TARGET";
const name = "Camera Add Follow Target";
const groups = ["EVENT_GROUP_CAMERA"];
const subGroups = { EVENT_GROUP_CAMERA: "EVENT_GROUP_ACTOR" };

const autoLabel = (fetchArg, input) =>
  input.useIndex
    ? `Camera also follow actor index ${fetchArg("actorIndex")}`
    : `Camera also follow ${fetchArg("actorId")}`;

const fields = [
  {
    label:
      "Adds one more actor to the camera's follow list without clearing the actors already in it. Use it after a Camera Lock event to follow more than four actors, or to build the list up in a loop. Up to 8 targets are kept.",
  },
  {
    key: "useIndex",
    label: "Select actor by index",
    description:
      "Choose the actor from a value or variable holding its scene index instead of picking it from the scene.",
    type: "checkbox",
    defaultValue: false,
  },
  {
    ...
    type: "actor",
    defaultValue: "$self$",
    conditions: [{ key: "useIndex", ne: true }],
  },
  {
    key: "actorIndex",
    label: "Actor Index",
    ...
    conditions: [{ key: "useIndex", eq: true }],
  },
];

const compile = (input, helpers) => {
  const { _addComment, _addNL } = helpers;
  _addComment("Camera Add Follow Target");
  compileTargetAdd(
    input.useIndex
...
module.exports = {
  ...
  name,
  description:
    "Adds an actor to the camera's follow list. The camera tracks the median position of every actor in the list.",
  autoLabel,
  groups,
  subGroups,
  fields,
