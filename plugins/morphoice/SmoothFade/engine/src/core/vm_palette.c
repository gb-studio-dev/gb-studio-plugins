...
void vm_load_palette(SCRIPT_CTX * THIS, UBYTE mask, UBYTE options) OLDCALL BANKED {
    ...
    for (UBYTE i = mask, nb = 0; (i != 0); dest++, nb++, i >>= 1) {
            ...
            switch (nb) {
                case 0:
                    ...
                    if (is_bkg) {
                        ...
                        if (is_commit && !fade_timer) BGP_REG = DMGPal;
                    }
                    if (is_spr) {
                        DMG_palette[1] = DMGPal;
                        if (is_commit && !fade_timer) OBP0_REG = DMGPal;
                    }
                    break;
                case 1:
                    if (is_spr) {
                        ...
                        if (is_commit && !fade_timer) OBP1_REG = DMGPal;
                    }
                    break;
            }
        }
        if (is_commit) {
            #ifdef CGB
                if (_is_CGB) {
                    // Skip hardware commit when faded — the software palette
                    // (BkgPalette/SprPalette) is already updated above.
                    // fade_applypalettechange() at the end will write the
                    // correctly interpolated colors to hardware.
                    if (!fade_timer) {
    ...
    // Re-apply current fade state after palette commit so that raw colors
    // don't flash through when the screen is faded out or mid-fade
    #ifdef CGB
        if (_is_CGB && is_commit && fade_timer) {
